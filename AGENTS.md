...
**Output guarding:** `mcp-output-guard.ts` is the module that bounds what an MCP server can push into the model's context. `guardMcpOutput` head-truncates text over `DEFAULT_MCP_OUTPUT_MAX_BYTES` (50 KiB), `DEFAULT_MCP_OUTPUT_MAX_LINES` (2000), or `DEFAULT_MCP_OUTPUT_MAX_TOKENS` (10,000 estimated tokens, at 4 chars/token) — whichever trips first — appends a pointer notice, and spills the full payload to a mode-`0600` file under `<agent dir>/mcp-output/` (temp-dir fallback if the agent dir is unwritable); `details.mcpResult` is kept raw only while its JSON stays under `DEFAULT_MCP_DETAILS_MAX_BYTES` (16 KiB), and is otherwise replaced by a summary. Image blocks pass through untouched. The token cap is configurable via `settings.outputGuard.maxTokens` or `MCP_OUTPUT_MAX_TOKENS` (`0` = off); `MCP_OUTPUT_GUARD=0` disables the guard entirely.
